...
export const TrelloKanban = () => {
  ...
  const handleDragEnd = async (result) => {
    ...
    const sourceColId = isNaN(source.droppableId) ? source.droppableId : parseInt(source.droppableId);
    const destColId = isNaN(destination.droppableId) ? destination.droppableId : parseInt(destination.droppableId);
    const taskId = isNaN(draggableId) ? draggableId : parseInt(draggableId);

    // Optimistic UI update
    const newColumns = [...columns];
    const sourceColIndex = newColumns.findIndex((c) => String(c.id) === String(sourceColId));
    const destColIndex = newColumns.findIndex((c) => String(c.id) === String(destColId));
  ...
  const handleSaveTask = async (taskData) => {
    ...
    try {
      if (taskData.id) {
        ...
        setColumns((prev) =>
          prev.map((col) => {
            if (String(col.id) === String(taskData.column_id)) {
              return {
                ...col,
                tasks: col.tasks.map((t) => (t.id === taskData.id ? updated : t)),
              };
            } else {
              return {
                ...col,
        ...
        // Create
        const targetCol = taskData.column_id || targetColumnForNewTask || columns[0]?.id;
        ...
        if (created) {
          setColumns((prev) =>
            prev.map((col) => {
              if (String(col.id) === String(created.column_id)) {
  ...
  const handleAddColumn = async (e) => {
    ...
    try {
      const createdCol = await createColumn(
        board.id,
        newColumnName.trim(),
        columns.length
      );
      if (createdCol) {
        setColumns((prev) => [...prev, createdCol]);
      }
      setNewColumnName('');
  ...
  const handleRenameColumn = async (colId) => {
    ...
    try {
      await updateColumn(colId, editingColumnName.trim());
      setColumns((prev) =>
