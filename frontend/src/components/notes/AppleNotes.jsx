...
export const AppleNotes = () => {
  ...
  const fetchNotes = async (selectFirst = false) => {
    try {
      ...
      if (res.data.folders && res.data.folders.length > 0) {
        // combine default folders with unique ones
        const unique = Array.from(new Set(['Notes', ...res.data.folders]));
        setFolders(unique);
      }

      if (selectFirst && res.data.notes.length > 0) {
  ...
  const triggerAutoSave = (newTitle, newContent, newFolder) => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        const res = await api.put(`/notes/${selectedNoteId}`, {
          title: newTitle || 'Untitled Note',
          content: newContent,
          folder: newFolder,
        });

        // Update in local notes list
        setNotes((prev) =>
          prev.map((item) => (item.id === selectedNoteId ? res.data.note : item))
        );
        setSaveStatus('saved');
        refreshUserStats();
      } catch (err) {
  ...
  const handleTogglePin = async (noteId, e) => {
    ...
    try {
      ...
      setNotes((prev) =>
        prev.map((n) => (n.id === noteId ? res.data.note : n))
      );
